...
@WebServlet("/product/play/playImgUpdate")
public class PlayImgUpdateServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private PlayService playService = new PlayService();
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String playId = request.getParameter("playId");
		
		List<ImageStore> list = playService.selectImageStore(playId);
		Play play = playService.selectPlayOne(playId);
		
		request.setAttribute("play", play);
		request.setAttribute("list", list);
		request.getRequestDispatcher("/WEB-INF/views/product/play/playImgUpdate.jsp").forward(request, response);
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//MultipartRequest객체 생성
				...
				while(fileNames.hasMoreElements()) {
					...
					result += playService.playImgUpdate(imageStore);
					i--;
				}
