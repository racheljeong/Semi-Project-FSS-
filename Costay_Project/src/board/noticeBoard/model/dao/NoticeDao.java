package board.noticeBoard.model.dao;
...
public class NoticeDao {
	...
	public NoticeDao() {
		...
		String fileName = "/sql/board/noticeBoard-query.properties";  //prop에 담을 내용이 있는 properties파일의 경로를 지정해 문자열로 지정해둠
		String path = NoticeDao.class.getResource(fileName).getPath();   //getPath이전은 url객체가 나옴 -> 절대경로로 바꿔줌
		...
		public int selectTotalNoticeBoards(Connection conn) {
			
			PreparedStatement pstmt = null;
			ResultSet rset = null;
			String sql = prop.getProperty("selectTotalNoticeBoards");
		...
		public Board selectNoticeBoardNo(Connection conn, int boardNo) {
			Board board = null;
			PreparedStatement pstmt = null;
			ResultSet rset = null;
			String sql = prop.getProperty("selectNoticeBoardNo");
		...
		public int updateNoticeBoard(Connection conn, Board board) {
			int result = 0;
			PreparedStatement pstmt = null;
			...
			try {
				...
				//DML은 executeUpdate()
				result = pstmt.executeUpdate();
				System.out.println("updateDao = " + result);
		...
		public int updateNoticeReadCount(Connection conn, int boardNo) {
			PreparedStatement pstmt = null;
			int result = 0;
		...
		public List<Board> selectBoardsBy(Connection conn, Map<String, Object> param) {
			...
			try{
				...
				while(rset.next()){
					...
					noticeList.add(board);
					System.out.println("boardFinder@dao/noticeList = " + noticeList); 
				}
			}catch(Exception e){
				e.printStackTrace();
			}finally{
