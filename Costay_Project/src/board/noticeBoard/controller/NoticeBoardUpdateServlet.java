 ...
 */
@WebServlet("/board/notice/boardUpdate")
public class NoticeBoardUpdateServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//2.businessLogic
		Board b = noticeService.selectNoticeBoardNo(boardNo);
		
		//3.view
		HttpSession session = request.getSession();
		session.setAttribute("title", "고객센터");
		request.setAttribute("board", b);
		request.getRequestDispatcher("/WEB-INF/views/board/noticeBoard/noticeBoardUpdate.jsp").forward(request, response);
		System.out.println("update@servlet = " + boardNo);
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Board board = new Board(boardNo, boardTitle, boardWriter, boardContent, null, 0, boardOriginalFileName, boardRenamedFileName, "N", 'N');
		
		//2. 업무로직 : Board객체 db수정 요청
		int result = noticeService.updateNoticeBoard(board);
		String msg = result > 0 ? "게시글 수정 성공!" : "게시글 수정 실패!";
		String loc = result > 0 ?
				request.getContextPath() + "/board/notice/boardView?boardNo=" + boardNo :
					
					request.getContextPath() + "/board/notice/BoardList";
//		String location = request.getContextPath() + "/board/noticeBoard/boardView?boardNo=" + boardNo;
