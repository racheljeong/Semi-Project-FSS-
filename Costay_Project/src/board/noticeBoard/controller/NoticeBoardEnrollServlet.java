 ...
 */
@WebServlet("/board/notice/boardEnroll")
public class NoticeBoardEnrollServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		request.getRequestDispatcher("/WEB-INF/views/board/noticeBoard/noticeBoardEnroll.jsp")	
	...
	//리스트의 토탈 컨텐츠는 전체 파인더는 해당 검색에 대한 회원수 또는 게시글 수 셀렉스멤버스바이와 셀렉트토탈멤버스바이는 동일한 where절을 갖는다
		...
		*/
		...
		int result = noticeService.insertNoticeBoard(board);
...
//		System.out.println("saveDirectory@servlet=" + saveDirectory);
		...
		String location = request.getContextPath() + "/board/notice/boardList";
