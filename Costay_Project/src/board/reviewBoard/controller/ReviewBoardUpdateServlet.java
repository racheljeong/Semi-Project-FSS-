 ...
 */
@WebServlet("/board/review/boardUpdate")
...
//		System.out.println("@@@@@@@@@@@");
		...
		request.getRequestDispatcher("/WEB-INF/views/board/reviewBoard/reviewBoardUpdate.jsp").forward(request, response);
	}
...
//			System.out.println(delFile + ":" + (bool ? "기존파일삭제 성공!" : "기존파일 삭제 실패"));
		...
		int result = reviewService.reviewUpdate(board);
		System.out.println("result@servlet="+result);
		
		String msg = result > 0 ? "게시물 수정 성공!" : "게시물 수정 실패";
		
		String loc = result > 0 ?
				request.getContextPath() + "/board/review/boardView?boardNo=" + boardNo :
					
					request.getContextPath() + "/board/review/BoardList";  
		...
		//////////////////////마지막 경로에서 오류ㅠㅠㅠㅠㅠㅠㅠㅠㅠㅠㅠㅠㅠㅠㅠㅠㅠㅠㅠㅠㅠㅠㅠ
	}
