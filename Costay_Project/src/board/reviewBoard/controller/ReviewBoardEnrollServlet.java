 ...
 */
@WebServlet("/board/review/boardEnroll")
public class ReviewBoardEnrollServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		request.getRequestDispatcher("/WEB-INF/views/board/reviewBoard/reviewBoardEnroll.jsp")	
	...
	//리스트의 토탈 컨텐츠는 전체 파인더는 해당 검색에 대한 회원수 또는 게시글 수 셀렉스멤버스바이와 셀렉트토탈멤버스바이는 동일한 where절을 갖는다
		...
		*/
		...
		Board board = new Board(0, boardTitle, boardWriter, boardContent, null, 0, boardOriginalFileName, boardRenamedFileName, "R", 'N');
		...
		int result = reviewService.insertReviewBoard(board);
//		System.out.println("result@servlet=" + result);
		...
		String location = request.getContextPath() + "/board/review/boardList";
