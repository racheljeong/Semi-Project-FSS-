package board.newHostBoard.model.dao;
...
public class NewHostDao {
	
	private Properties prop = new Properties();
	
public NewHostDao() {
		
		String fileName = "/sql/board/newHostBoard-query.properties";  //prop에 담을 내용이 있는 properties파일의 경로를 지정해 문자열로 지정해둠
		String path = NewHostDao.class.getResource(fileName).getPath();   //getPath이전은 url객체가 나옴 -> 절대경로로 바꿔줌
	...
	public List<BoardExt> selectBoardsBy(Connection conn, Map<String, Object> param) {
		List<BoardExt> hostList = new ArrayList<>();
		...
		try{
			...
			while(rset.next()){
				...
				board.setBoardDelFlag(rset.getString("board_del_flag").charAt(0));
				 ...
				 hostList.add(board);
				System.out.println("boardFinder@dao/reviewList = " +  hostList); 
			}
		}catch(Exception e){
			e.printStackTrace();
		}finally{
	...
	public int insertNewHostBoard(Connection conn, Board board) {
		//글작성
		PreparedStatement pstmt = null;
		int result = 0;
		String sql = prop.getProperty("insertNewHostBoard");
	...
	public int selectLastBoardNo(Connection conn) {
		...
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		String sql = prop.getProperty("selectLastNHBoardNo");
		int boardNo = 0;
	...
	public int updateNewHostReadCount(Connection conn, int boardNo) {
		//조회수 올리기
		PreparedStatement pstmt = null;
		int result = 0;
		String sql = prop.getProperty("updateNewHostReadCount");
		...
		//System.out.println("result-updateNewHostReadCount@dao="+result); 잘나옴
		return result;
	}


	public Board selectNewHostBoardNo(Connection conn, int boardNo) {
		Board board = null;
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		String sql = prop.getProperty("selectNewHostBoardNo");
	...
	public List<BoardComment> newHostCommentList(Connection conn, int boardNo) {
		//댓글 리스트
		List<BoardComment>commentList = null;
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		String query = prop.getProperty("newHostCommentList");
	...
	public int deleteNewHostComment(Connection conn, int boardCommentNo) {
		PreparedStatement pstmt = null;
		int result = 0;
		String query = prop.getProperty("deleteNewHostComment");
	...
	public int insertNewHostComment(Connection conn, BoardComment bc) {
		//댓글등록
		PreparedStatement pstmt = null;
		int result = 0;
		String query = prop.getProperty("insertNewHostComment"); 
		//insertReviewComment = 
	...
	public List<BoardExt> selectNewHostList(Connection conn, int cpage, int numPerPage) {
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		List<BoardExt>hostList = new ArrayList<>();
		
		String sql = prop.getProperty("selectNewHostList");
...
//				Board board = new Board();
	...
	public int selectTotalNHBoards(Connection conn) {
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		String sql = prop.getProperty("selectTotalNHBoards");
	...
	public int newHostUpdate(Connection conn, Board board) {
		//게시물 수정
		PreparedStatement pstmt = null;
		int result = 0;
		String query = prop.getProperty("newHostUpdate"); 
	...
	public int deleteNewHostBoard(Connection conn, int boardNo) {
		PreparedStatement pstmt = null;
		int result = 0;
		String query = prop.getProperty("deleteNewHostBoard"); 
		// deleteNewHostBoard = delete from board where board_no = ?
		try {
			//미완성쿼리문을 가지고 객체생성.
			pstmt = conn.prepareStatement(query);
			pstmt.setInt(1, boardNo);
