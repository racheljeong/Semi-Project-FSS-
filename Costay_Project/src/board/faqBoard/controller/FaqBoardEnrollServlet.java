 ...
 */
@WebServlet("/board/faq/boardEnroll")
public class FaqBoardEnrollServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		request.getRequestDispatcher("/WEB-INF/views/board/faqBoard/faqBoardEnroll.jsp")	
	...
	//리스트의 토탈 컨텐츠는 전체 파인더는 해당 검색에 대한 회원수 또는 게시글 수 셀렉스멤버스바이와 셀렉트토탈멤버스바이는 동일한 where절을 갖는다
		...
		*/
		...
		Board board = new Board(0, boardTitle, boardWriter, boardContent, null, 0, boardOriginalFileName, boardRenamedFileName, "F", 'N');
		...
		int result = faqService.insertFaqBoard(board);
		
		String msg = result > 0 ? "게시글 등록 성공!" : "게시글 등록 실패";
		
		// 성공시 상세보기 실패시 목록페이지로 이동
		//board.getBoardNo() 를 통해 새로 등록된 게시글 번호 가져오기
		String location = request.getContextPath() + "/board/faq/boardList";
