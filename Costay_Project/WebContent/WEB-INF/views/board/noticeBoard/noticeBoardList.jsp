...
<div class="sub_title">
 <h3 style="margin-top:40px;"><strong>Notice Board</strong></h3>
 <hr />
</div>
	...
	<ul>
		...
		<div id="searchBoard" class="board">
	        ...
	        <div id="search-boardTitle" class="search-type">
	            <form action="<%=request.getContextPath()%>/board/notice/boardFinder">
       ...
       <%--  <div id="search-board_Enroll_date" class="search-type">
            <form action="<%=request.getContextPath()%>/board/notice/boardFinder">
...
<!--         </div> -->
	...
	<% if(noticeList == null || noticeList.isEmpty()) { %>
		...
		<tr>
			...
			<td>
				<%--제목 --%>
				<a href="<%=request.getContextPath()%>/board/notice/boardView?boardNo=<%=b.getBoardNo()%>">
					<%=b.getBoardTitle() %>
				</a>
			</td>
			<td>
	...
	<% if((memberLoggedIn != null)&&memberLoggedIn.getMemberRole()==MemberService.ADMIN_MEMBER_ROLE){ %>
			<input 
				...
				value="글쓰기" 
				id="write-btn"
				onclick="location.href='<%= request.getContextPath() %>/board/notice/boardEnroll';" />
		<% } %>
	</div>
