...
<%
	...
	System.out.println("commentList@viewJsp="+commentList);   ///왜 갑자기 잘나와???????????
...
$(function() {
 ...
 $(".btn-delete").click(function(){
	if(confirm('정말 삭제하시겠습니까?')){
		...
		location.href = "<%= request.getContextPath() %>/board/review/CommentDelete?boardCommentNo=" +boardCommentNo + "&boardNo=<%=board.getBoardNo() %>";
	}
});
	...
	*/
	 $(".btn-reply").one('click', function() {
		...
		html += '<form action="<%=request.getContextPath() %>/board/review/CommentInsert" method="post">';
...
function fileDownload(oName, rName){
	...
	location.href = "<%= request.getContextPath() %>/board/review/fileDownload?oName=" +oName + "&rName=" +rName;
}
...
<div style="margin:0 auto; text-align:center; width:800px;">
	<table border="1" class="table   table-bordered">
		...
		<tr style="text-align:left;">
			...
			<th colspan="2">
			<input type="button" value="수정하기" onclick="updateReviewBoard();" /> 
			<input type="button" value="삭제하기" onclick="deleteReviewBoard();" />
			<tr>
		</th>
		</tr>
		<script>
			...
			function updateReviewBoard() {
				location.href = "<%= request.getContextPath() %>/board/review/boardUpdate?boardNo=<%=board.getBoardNo()%>";
			/* 	location.href는 get방식 */
			}
			
			function deleteReviewBoard() {
		...
		<form 
			action="<%= request.getContextPath() %>/board/review/boardDelete"
			method="post" 
			name="boardDelFrm">
	...
	<div class="comment-container">
		...
		<form
			action="<%=request.getContextPath()%>/board/review/CommentInsert"
			method="post" 
	...
	<table id="tbl-comment" class="table   table-bordered">
		<%-- 댓글인 경우 tr.level1 --%>
