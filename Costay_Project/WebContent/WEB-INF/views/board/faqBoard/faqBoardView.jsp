...
function fileDownload(oName, rName){
	...
	location.href = "<%= request.getContextPath() %>/board/faq/fileDownload?oName=" + oName + "&rName=" + rName; 
}
...
<div style="margin:0 auto; text-align:center; width:800px;">
	<table border="1" class="table   table-bordered">
		...
		<script>
			function updateFaqBoard() {
				location.href = "<%= request.getContextPath() %>/board/faq/boardUpdate?boardNo=<%=b.getBoardNo()%>";
			...
			function deleteFaqBoard() {
				if(confirm("정말로 이 게시물을 삭제하시겠습니까?")) {
					$("[name=faqBoardDelFrm]").submit();
				}
			}
			</script>
			<form 
				action="<%= request.getContextPath() %>/board/faq/boardDelete"
				method="post"
				name="faqBoardDelFrm">
